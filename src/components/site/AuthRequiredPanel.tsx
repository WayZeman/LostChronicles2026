...
function errorMessage(code: string | null | undefined): string | null {
  if (!code) return null;
  if (code === "oauth" || code === "discord" || code === "google") {
    return "Не вдалося увійти. Спробуй ще раз або інший спосіб.";
  }
  if (code === "discord_config" || code === "google_config") {
    return "Вхід тимчасово недоступний. Спробуй пізніше.";
  }
  return null;
}
...
export function AuthRequiredPanel({
  ...
  return (
    <div className={cn(lcGlassPanelClass, "mx-auto max-w-lg text-center")}>
      ...
      {err ? (
        <p
          className="mt-4 rounded-[var(--radius)] border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-sm text-rose-100"
          role="alert"
        >
          {err}
        </p>
