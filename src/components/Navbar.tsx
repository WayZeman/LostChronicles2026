...
const links = [
  ...
  { href: "/map", label: "Мапа", Icon: MapIcon },
  {
    ...
    label: "Голосування",
    Icon: ClipboardList,
    restricted: "proposals" as const,
  },
  { href: "/wiki", label: "Вікі", Icon: BookOpen, restricted: "wiki" as const },
  ...
  return (
    <nav className="lc-bottom-nav mc-frame" aria-label="Головна навігація">
      <div className="mx-auto flex w-full items-stretch justify-around gap-0.5 px-1 md:px-2">
        {bottomNavLinks.map(({ href, label, Icon, ...rest }) => {
          const active = isActivePath(pathname, href);
          const restricted = "restricted" in rest ? rest.restricted : undefined;
          const className = cn(
